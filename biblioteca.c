...
public class Pacotes{
    ...
    FILE *pac;
    pac = fopen ("Pacote.txt", "r");

    if (pac == NULL){
        printf("Problema na abertura do arquivo Pacote\n");
        return 0;
    }

    while(fgets(text, 100, pac) != NULL){
            ...
            sub = strtok(NULL, ";");
            sub = strtok(NULL, ";");
            sepDados[13] = atof(sub);

    }

        double lat_1 = sepDados[0];
        double long_1 = sepDados[1];
        double lat_2 = sepDados[2];
        double long_2 = sepDados[3];
        double alt = sepDados[4];
        double vel = sepDados[5];
        double temp = sepDados[9];
        double pres = sepDados[13];
